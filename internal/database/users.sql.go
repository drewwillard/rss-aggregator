...
const getFollowsForUserName = `-- name: GetFollowsForUserName :many
...
type GetFollowsForUserNameRow struct {
	Name string
	Url  string
}

func (q *Queries) GetFollowsForUserName(ctx context.Context, name string) ([]GetFollowsForUserNameRow, error) {
	rows, err := q.db.QueryContext(ctx, getFollowsForUserName, name)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetFollowsForUserNameRow
	for rows.Next() {
		var i GetFollowsForUserNameRow
